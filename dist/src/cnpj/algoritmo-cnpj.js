 ...
 */
export function converterCaractereParaValor(caractere) {
 ...
 */
export function calcularDigitoVerificador(valores, pesos) {
 ...
 */
export function verificarSequenciaRepetida(valor) {
 ...
 */
export function validarCnpjPuro(puro) {
    if (puro.length !== TamanhoIdentificador.Total)
        return false;
    if (!/^[0-9A-Z]{12}[0-9]{2}$/.test(puro))
        return false;
    if (verificarSequenciaRepetida(puro))
        return false;
