...
class ErroOpenCnpjApi extends Error {
    constructor(statusHttp, mensagem) {
        super(mensagem);
 ...
 */
export async function validarCnpjIndividual(cnpj) {
    const puro = normalizarPuro(cnpj);
    if (puro.length !== 14) {
        return {
            puro,
 ...
 */
export async function validarCnpjsEmMassa(listaCnpjs) {
    const resultados = [];
...
class ValidadorCnpjApi {
    ...
    constructor(elementos) {
        ...
        this.escutarTeclaEscape = (evento) => {
     ...
     */
    configurarEventos() {
        const { botaoValidarUnico, botaoValidarMassa, controleMascara, controleMassa, botaoColar, modalOverlay, botaoFecharModal, } = this.elementos;
     ...
     */
    notificarResumoMassa(resultados) {
     ...
     */
    atualizarEstadoBotaoValidarMassa(total) {
        const { botaoValidarMassa } = this.elementos;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const elementos = {
        ...
        modalOverlay: obterElementoObrigatorio("modal-dados-cnpj"),
        modalConteudo: obterElementoObrigatorio("modal-conteudo-cnpj"),
        modalTitulo: obterElementoObrigatorio("modal-titulo-cnpj"),
