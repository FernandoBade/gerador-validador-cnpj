...
class GeradorCnpj {
        ...
        try {
            for (let i = 0; i < 10; i++) {
                if (this.historico.itens.length >= this.historico.limite) {
                    exibirAviso(this.elementos.areaAviso, `Limite de ${this.historico.limite} CNPJs atingido. CALMAAAAA QUE O NAVEGADOR NUM GUENTA!!! 😅`, TipoAviso.Erro);
                    break;
                }
    ...
    async tratarCliqueCopiar() {
        ...
        try {
            ...
            exibirAviso(this.elementos.areaAviso, `CNPJ copiado: ${valorParaCopiar}`, TipoAviso.InfoAlternativo);
        }
        catch (erro) {
            console.error(erro);
    ...
    gerarEExibirIdentificador(disparoManual = false, disparoEmLote) {
        ...
        if (this.historico.itens.length >= this.historico.limite) {
            exibirAviso(this.elementos.areaAviso, `Limite de ${this.historico.limite} CNPJs atingido. Não é possível gerar novos registros.`, TipoAviso.Erro);
        ...
        if (disparoEmLote)
            exibirAviso(this.elementos.areaAviso, `Novos ${disparoEmLote} CNPJs alfanuméricos gerados`, TipoAviso.Sucesso);
        ...
        this.historico.itens.forEach((puro) => {
            ...
            botao.addEventListener("click", async (evento) => {
                ...
                try {
                    ...
                    exibirAviso(this.elementos.areaAviso, `CNPJ copiado: ${texto}`, TipoAviso.InfoAlternativo);
                }
                catch {
                    exibirAviso(this.elementos.areaAviso, "Falha ao copiar", TipoAviso.Erro);
                }
    ...
    async copiarTodos() {
        ...
        try {
            ...
            if (this.historico.itens.length === 1) {
                ...
                exibirAviso(this.elementos.areaAviso, `Copiados ${this.historico.itens.length} CNPJs separados por vírgula`, TipoAviso.Info);
            }
        }
        catch {
