...
class GeradorCnpj {
    ...
    private gerarEExibirIdentificador(disparoManual = false, disparoEmLote?: number): void {
        ...
        if (disparoManual) exibirAviso(this.elementos.areaAviso, "Novo CNPJ alfanumérico gerado", TipoAviso.Sucesso);

        if (disparoEmLote) exibirAviso(this.elementos.areaAviso, `Novos ${disparoEmLote} CNPJs alfanuméricos gerados`, TipoAviso.Sucesso);
