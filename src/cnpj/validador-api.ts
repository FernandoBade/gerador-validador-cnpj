...
type ResultadoConsulta = InformacoesCnpj | ResultadoErro | null | undefined;

const mapaItens = new Map<string, HTMLLIElement>();
const mapaDadosModal = new Map<HTMLButtonElement, InformacoesCnpj>();
let ultimoBotaoAtivo: HTMLButtonElement | null = null;
let listenerEscape: ((evento: KeyboardEvent) => void) | null = null;
 ...
 */
export async function consultarCNPJ(cnpj: string): Promise<RegistroGenerico | null> {
    try {
        ...
        return (await resposta.json()) as RegistroGenerico;
    } catch (erro) {
...
function inicializarPagina(): void {
    const elementos: ElementosPaginaApi = {
        formulario: obterElementoObrigatorio<HTMLFormElement>("form-validacao-api"),
        campoCnpjs: obterElementoObrigatorio<HTMLTextAreaElement>("campo-cnpjs"),
        ...
        modal: obterElementoObrigatorio<HTMLDivElement>("modal-detalhes"),
        tituloModal: obterElementoObrigatorio<HTMLHeadingElement>("titulo-modal"),
        listaDetalhesModal: obterElementoObrigatorio<HTMLDListElement>("lista-detalhes-modal"),
 ...
 */
async function validarCnpjs(elementos: ElementosPaginaApi): Promise<void> {
    ...
    const promessas = entradas.map(async (cnpj) => {
        ...
        if (cnpj.length !== 14) {
            ...
            if (!exibiuAvisoFormato) {
                exibirAviso(
                    elementos.areaAviso,
                    "Alguns CNPJs possuem menos de 14 dígitos. Ajuste e tente novamente.",
                    TipoAviso.Info,
                );
        ...
        try {
            ...
            if (dadosBrutos === null) {
                ...
                exibirAviso(
                    elementos.areaAviso,
                    `CNPJ ${aplicarMascara(cnpj)} não foi encontrado no OpenCNPJ.`,
                    TipoAviso.Info,
                );
                return;
            }
 ...
 */
function limparResultados(elementos: ElementosPaginaApi): void {
 ...
 */
function obterCnpjsDoFormulario(valor: string): string[] {
    return valor
        .split(/[\s,;]+/)
 ...
 */
function exibirResultado(cnpj: string, dados: ResultadoConsulta, elementos: ElementosPaginaApi): HTMLLIElement {
    const chave = cnpj;
    ...
    const botao = item.querySelector<HTMLButtonElement>("button");
    const status = item.querySelector<HTMLSpanElement>(".status-resultado");
    const mensagem = item.querySelector<HTMLParagraphElement>(".mensagem-resultado");
    const descricaoCnpj = item.querySelector<HTMLSpanElement>(".descricao-cnpj");
 ...
 */
function abrirModal(dadosCNPJ: InformacoesCnpj, elementos: ElementosPaginaApi): void {
    elementos.tituloModal.textContent = `CNPJ ${dadosCNPJ.cnpjFormatado}`;

    const campos: Array<{ titulo: string; valor: string }> = [
    ...
    listenerEscape = (evento: KeyboardEvent) => {
 ...
 */
function fecharModal(elementos: ElementosPaginaApi): void {
 ...
 */
function ehErroResultado(dados: ResultadoConsulta): dados is ResultadoErro {
 ...
 */
function criarIndicador(classesExtras: string): HTMLSpanElement {
 ...
 */
function criarTextoStatus(texto: string): HTMLSpanElement {
    const span = document.createElement("span");
 ...
 */
function criarSpinner(): HTMLSpanElement {
    const span = document.createElement("span");
 ...
 */
function extrairInformacoesCnpj(dados: RegistroGenerico, cnpj: string): InformacoesCnpj {
 ...
 */
function obterRegistro(fonte: RegistroGenerico | undefined, chave: string): RegistroGenerico | undefined {
    if (!fonte) return undefined;
    const valor = fonte[chave];
    if (typeof valor === "object" && valor !== null && !Array.isArray(valor)) {
        return valor as RegistroGenerico;
    }
    return undefined;
}
 ...
 */
function obterTexto(fontes: Array<RegistroGenerico | undefined>, chaves: string[]): string | undefined {
 ...
 */
function normalizarValorTexto(valor: unknown): string | undefined {
 ...
 */
function extrairCnaePrincipal(
    estabelecimento: RegistroGenerico | undefined,
    dados: RegistroGenerico,
): string | undefined {
...
function formatarData(valor: string | undefined): string {
    ...
    if (apenasNumeros.length === 8) {
        ...
        if (anoInicial >= 1900) {
            const data = new Date(
                `${apenasNumeros.slice(0, 4)}-${apenasNumeros.slice(4, 6)}-${apenasNumeros.slice(6, 8)}`,
            );
            if (!Number.isNaN(data.getTime())) {
                return formatadorData.format(data);
            }
 ...
 */
function formatarCapital(valor: string | undefined): string {
    if (!valor) {
